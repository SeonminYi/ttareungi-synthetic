...
# ===============================
# 5. 프롬프트 템플릿 (CONTENT_PHRASES 없이 사용)
# ===============================
...
- When an object overlaps the metal grid, the wires must stay clearly visible in front of the object
  (the object is occluded by the grid, never painting over the wires).
...
def build_prompt(content_type: str, intensity: int) -> str:
    """
    ver2에서 사용하던 CONTENT_PHRASES를 제거하고,
    object 예시 + 간단 지시문만 사용하는 버전.
    """
    intensity_phrase = intensity_to_phrase(intensity)

    # 기본 지시문 (content_type 공통)
